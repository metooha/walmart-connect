...
export default function DisplayAdvertisingCampaigns() {
  ...
  const getStatusBadge = (status: Campaign["status"]) => {
    const colorMap: Record<string, "green" | "blue" | "purple" | "gray"> = {
      Live: "green",
      Scheduled: "blue",
      Paused: "purple",
      Completed: "gray"
    ...
    const color = selectedTab === "archive" ? "gray" : colorMap[status];
  ...
  return (
    <div className="min-h-screen bg-background">
      ...
      <MastHead
        ...
        onSolutionChange={(solution) => setSelectedMediaSolution(solution)}
      />
          ...
          <div className="mx-3 mb-6 rounded-lg shadow-[0_-1px_2px_0_rgba(0,0,0,0.10),0_1px_2px_1px_rgba(0,0,0,0.15)] overflow-hidden flex flex-col flex-1">
            ...
            <div className="flex items-center justify-end gap-2 p-4 border-b border-border bg-background">
                ...
                <div className="relative">
                  ...
                  {showSearchScopeDropdown && (
                    <div
                      ...
                      className="absolute left-0 top-full mt-1 w-[160px] bg-background rounded border border-[#D5D6D8] shadow-[0_-1px_4px_0_rgba(0,0,0,0.10),0_5px_10px_3px_rgba(0,0,0,0.15)] z-50 py-1"
                    >
                      <button
              ...
              <div className="flex items-center gap-2 relative">
                {/* All Filters Button */}
                <button
                  className={`flex items-center justify-center h-8 w-8 px-1.5 border rounded-full transition-all focus:outline-none focus:ring-2 focus:ring-primary focus:ring-opacity-50 ${
                    showAllFiltersPopover || statusFilter.length > 0 || livePacingFilter.length > 0 || completedFilterSelected || searchQuery.trim()
                      ? 'border-2 border-primary bg-primary/10'
                      : 'border-[rgba(46,47,50,1)] bg-background hover:bg-muted'
                  }`}
                ...
                <div className="relative">
                  <button
                    className={`flex items-center gap-1 h-8 pl-3 pr-2 border rounded-full transition-all focus:outline-none focus:ring-2 focus:ring-primary focus:ring-opacity-50 ${
                    ...
                    aria-haspopup="true"
                  >
                    <span className="text-sm text-foreground" aria-hidden="true">Pace</span>
                    {livePacingFilter.length > 0 && (
                      <span className="text-sm text-foreground" aria-hidden="true">({livePacingFilter.length})</span>
                    )}
                    {showLiveFilterPopover ? (
                      <ChevronUp className="w-4 h-4 text-foreground" aria-hidden="true" />
                    ) : (
                ...
                <button
                  className={`flex items-center gap-1 h-8 px-3 border rounded-full transition-all focus:outline-none focus:ring-2 focus:ring-primary focus:ring-opacity-50 ${
                ...
                >
                  <span className="text-sm text-foreground" aria-hidden="true">Completed</span>
                </button>
              </div>
                ...
                <tbody>
                  {getPaginatedCampaigns().map((campaign, idx) => (
                    <Fragment key={campaign.id}>
                      <tr className={`border-b border-border hover:bg-blue-50 group ${
                        ...
                        <td className={`p-2 sticky left-0 group-hover:bg-blue-50 z-10 shadow-[2px_0_4px_-2px_rgba(0,0,0,0.1)] ${
                          searchQuery.trim() && (campaign.name.toLowerCase().includes(searchQuery.toLowerCase()) || campaign.id.toLowerCase().includes(searchQuery.toLowerCase()))
                            ? 'bg-amber-50'
                            : 'bg-background'
                        ...
                        <td className="p-2" style={{ width: columnWidths.campaign }}>
                          <div className="flex items-start gap-1">
                            ...
                            <div className="flex-1">
                              <div className="text-foreground underline hover:no-underline cursor-pointer">
                                {campaign.name}
                              </div>
                        ...
                        <td className={`p-2 sticky right-0 group-hover:bg-blue-50 z-10 shadow-[-2px_0_4px_-2px_rgba(0,0,0,0.1)] ${
                          searchQuery.trim() && (campaign.name.toLowerCase().includes(searchQuery.toLowerCase()) || campaign.id.toLowerCase().includes(searchQuery.toLowerCase()))
                            ? 'bg-amber-50'
                            : 'bg-background'
                      ...
                      {campaign.expanded && campaign.children?.map((child, childIdx) => (
                        <tr key={child.id} className="border-b border-border bg-background hover:bg-blue-50 group">
                          <td className="p-2 sticky left-0 bg-background group-hover:bg-blue-50 z-10 shadow-[2px_0_4px_-2px_rgba(0,0,0,0.1)]" style={{ width: columnWidths.checkbox }}></td>
                          <td className="p-2 pl-12" style={{ width: columnWidths.campaign }}>
                            <div className="flex items-center gap-2">
                              ...
                              <span className="text-foreground underline hover:no-underline cursor-pointer">
                                {child.name}
                              </span>
                            </div>
                          </td>
                          ...
                          <td className="p-2 sticky right-0 bg-background group-hover:bg-blue-50 z-10 shadow-[-2px_0_4px_-2px_rgba(0,0,0,0.1)]" style={{ width: columnWidths.actions }}>
                            <button className="p-2 hover:bg-muted rounded-full">
                              <MoreHorizontal className="w-4 h-4" />
                            </button>
                          </td>
