...
const STORAGE_KEY = 'panel-resize-width';
  ...
  (
    ...
    React.useEffect(() => {
      if (resizable) {
        ...
        if (stored) setResizeWidth(stored);
      }
    }, [resizable]);
    ...
    const handleResizeStart = (e: React.MouseEvent) => {
      ...
      const handleMouseUp = () => {
        ...
        // Persist
        setResizeWidth(prev => { if (prev) storeWidth(prev); return prev; });
      };
    ...
    const panelStyle: React.CSSProperties = resizable && resizeWidth
      ? { width: resizeWidth }
      : {};
    ...
    const panelContent = (
      <>
        ...
        <div
          ref={panelRef}
          className={`${styles.panel} ${sizeClass} ${positionClass} ${stateClass} ${resizable ? styles['panel--resizable'] : ''} ${className || ''}`}
          role="dialog"
          aria-modal="true"
        ...
        >
          ...
          {resizable && (
            <div
              className={`${styles.panel__resizeHandle} ${position === 'left' ? styles['panel__resizeHandle--right'] : ''}`}
              onMouseDown={handleResizeStart}
              role="separator"
              aria-orientation="vertical"
              aria-label="Resize panel"
            >
              <div className={styles.panel__resizeIndicator} />
            </div>
          )}
